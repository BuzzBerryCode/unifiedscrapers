...
def calculate_change(new_value, old_value):
    """Calculates the difference and determines the change type, returning an integer."""
    if old_value is None or new_value is None:
        return 0, 'zero'
    
    # Ensure values are numeric for calculation
    try:
        # ** FIX: Calculate as float, then explicitly cast to int to match the database **
        change = int(float(new_value) - float(old_value))
...
def scrape_user_data(username):
    ...
    # --- AI Predictions based on Primary Niche ---
    ...
    location = predict_freeform_location(bio, all_captions, all_locations)
...
def rescrape_and_update_creator(handle, existing_data):
    ...
    if not new_data or new_data.get('skipped'):
        print(f"ℹ️ No new data or skipped, aborting update for @{handle}.")
        return
...
if __name__ == "__main__":
    # --- PHASE 1: RESCRAPE AND UPDATE EXISTING CREATORS ---
    print("🚀 STARTING SCRIPT 🚀")
    print("\n" + "="*50)
    print("PHASE 1: UPDATING EXISTING CREATORS FROM DATABASE")
    print("="*50)
    ...
    if not existing_creators:
        print("No existing 'Trading' creators found in the database to update.")
    else:
        for creator_data in existing_creators:
            rescrape_and_update_creator(creator_data['handle'], creator_data)

    # --- PHASE 2: PROCESS NEW CREATORS FROM EXCEL ---
    print("\n" + "="*50)
    ...
    if not os.path.exists(excel_path):
        print(f"❌ Error: File '{excel_file}' not found. Skipping Phase 2.")
    else:
        try:
            df = pd.read_excel(excel_path)
            if 'Usernames' not in df.columns:
                print("❌ Error: Excel file must have a 'Usernames' column. Skipping Phase 2.")
            else:
