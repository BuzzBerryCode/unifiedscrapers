...
async def process_creator_media(creator_id: str, handle: str, creator_data: dict):
    ...
    if creator_data.get("profile_image_url"):
        ...
        print(f"🖼️ Downloading latest profile image for @{handle}...")
    ...
    for i in range(1, MAX_RECENT_POSTS + 1):
        ...
        try:
            ...
            print(f"Error processing media URL {media_url}: {e}")
            continue
...
async def main():
    ...
    # --- PHASE 1: UPDATE EXISTING CREATORS ---
    print("\n" + "="*50)
    print("PHASE 1: UPDATING EXISTING 'TIKTOK' CREATORS FROM DATABASE")
    print("="*50)
    ...
    if not existing_creators:
        print("No existing 'TikTok' creators found to update.")
    else:
        for creator_data in tqdm(existing_creators, desc="Updating existing creators"):
            await rescrape_and_update_creator(creator_data['handle'], creator_data)
